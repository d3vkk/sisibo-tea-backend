...
const { hash, createRandomString, sendTwilioSms, msToTime } = require('./helpers')
  ...
  run(`SELECT * FROM teadata ORDER BY id DESC LIMIT 1`, (err, data)=>{
    if(!err){
      if(data.result.length > 0){
        ...
        console.log('red:', result.red, config.fermentedValues.red, 'blue:', result.blue, config.fermentedValues.blue, 'green:', result.green, config.fermentedValues.green)
        if(result.red == config.fermentedValues.red && result.green == config.fermentedValues.green && result.blue == config.fermentedValues.blue){
          sendTwilioSms(config.twilio.to, 'RGB values edging closer to fermentation values', err=>{
            if(err){
              console.log(err)
            }
